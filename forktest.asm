  ...
  80:	0f 84 7c 00 00 00    	je     102 <forktest+0xb2>
{
  ...
  9e:	66 90                	xchg   %ax,%ax
    if(wait() < 0){
  a0:	e8 25 03 00 00       	call   3ca <wait>
  ...
  if(wait() != -1){
  ae:	e8 17 03 00 00       	call   3ca <wait>
...
printf(int fd, char *s, ...)
{
  ...
  ce:	6a 01                	push   $0x1
  d0:	e8 0d 03 00 00       	call   3e2 <write>
...
printf(int fd, char *s, ...)
{
  ...
  f6:	6a 01                	push   $0x1
  f8:	e8 e5 02 00 00       	call   3e2 <write>
      exit();
  }
  ...
  if(n == N){
    printf(1, "fork claimed to work N times!\n", N);
    exit();
  fd:	e8 c0 02 00 00       	call   3c2 <exit>
  for(n=0; n<N; n++){
    pid = fork();
    if(pid < 0)
      break;
    if(pid == 0)
      exit();
 102:	e8 bb 02 00 00       	call   3c2 <exit>
#define N  1000
...
printf(int fd, char *s, ...)
{
 ...
 11d:	6a 01                	push   $0x1
 11f:	e8 be 02 00 00       	call   3e2 <write>
  }
  ...
  for(; n > 0; n--){
    if(wait() < 0){
      printf(1, "wait stopped early\n");
      exit();
 124:	e8 99 02 00 00       	call   3c2 <exit>
#define N  1000
...
printf(int fd, char *s, ...)
{
 ...
 13f:	6a 01                	push   $0x1
 141:	e8 9c 02 00 00       	call   3e2 <write>
    }
  }
  ...
  if(wait() != -1){
    printf(1, "wait got too many\n");
    exit();
 146:	e8 77 02 00 00       	call   3c2 <exit>
...
gets(char *buf, int max)
{
 ...
 275:	57                   	push   %edi
 276:	6a 00                	push   $0x0
 278:	e8 5d 01 00 00       	call   3da <read>
    if(cc < 1)
...
stat(char *n, struct stat *st)
{
 ...
 2dd:	e8 20 01 00 00       	call   402 <open>
  if(fd < 0)
 ...
 2f1:	50                   	push   %eax
 2f2:	e8 23 01 00 00       	call   41a <fstat>
 2f7:	89 c6                	mov    %eax,%esi
  close(fd);
 2f9:	89 1c 24             	mov    %ebx,(%esp)
 2fc:	e8 e9 00 00 00       	call   3ea <close>
  return r;
...
atoi(const char *s)
{
 ...
 325:	8b 5d 08             	mov    0x8(%ebp),%ebx
  ...
  n = 0;
	...
	if(*s=='-'){
 328:	0f be 13             	movsbl (%ebx),%edx
 32b:	80 fa 2d             	cmp    $0x2d,%dl
 32e:	74 38                	je     368 <atoi+0x48>
		sign=1;
		s++;
	}

  while('0' <= *s && *s <= '9')
 330:	8d 42 d0             	lea    -0x30(%edx),%eax
...
atoi(const char *s)
{
 ...
 333:	31 f6                	xor    %esi,%esi
  ...
  while('0' <= *s && *s <= '9')
 335:	3c 09                	cmp    $0x9,%al
 337:	77 47                	ja     380 <atoi+0x60>
	int sign=0;	// let positive as 0, negative as 1
 ...
 346:	8d 44 42 d0          	lea    -0x30(%edx,%eax,2),%eax
  ...
  while('0' <= *s && *s <= '9')
 34a:	0f be 13             	movsbl (%ebx),%edx
 34d:	8d 4a d0             	lea    -0x30(%edx),%ecx
 350:	80 f9 09             	cmp    $0x9,%cl
 353:	76 eb                	jbe    340 <atoi+0x20>
 355:	89 c2                	mov    %eax,%edx
 357:	f7 da                	neg    %edx
 359:	83 fe 01             	cmp    $0x1,%esi
 35c:	0f 44 c2             	cmove  %edx,%eax
  if(sign==1){
		return n*(-1);
	}
 ...
 364:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  ...
  while('0' <= *s && *s <= '9')
 368:	0f be 53 01          	movsbl 0x1(%ebx),%edx
  ...
  n = 0;
	...
	if(*s=='-'){
		sign=1;
		s++;
 36c:	8d 43 01             	lea    0x1(%ebx),%eax
	}

  while('0' <= *s && *s <= '9')
 36f:	8d 4a d0             	lea    -0x30(%edx),%ecx
 372:	80 f9 09             	cmp    $0x9,%cl
 375:	77 09                	ja     380 <atoi+0x60>
 ...
 377:	89 c3                	mov    %eax,%ebx
  ...
  n = 0;
	...
	if(*s=='-'){
		sign=1;
 379:	be 01 00 00 00       	mov    $0x1,%esi
 37e:	eb b9                	jmp    339 <atoi+0x19>
		s++;
	}

  while('0' <= *s && *s <= '9')
 380:	31 c0                	xor    %eax,%eax
 382:	eb db                	jmp    35f <atoi+0x3f>
