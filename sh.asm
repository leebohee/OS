...
fork1(void)
{
  ...
  exit();
      f5:	e8 58 0c 00 00       	call   d52 <exit>
      if(chdir(buf+3) < 0)
        printf(2, "cannot cd %s\n", buf+3);
      continue;
    }
...
runcmd(struct cmd *cmd)
{
  ...
  case PIPE:
    ...
    if(pipe(p) < 0)
     ...
     1a6:	50                   	push   %eax
     1a7:	e8 b6 0b 00 00       	call   d62 <pipe>
...
fork1(void)
{
  ...
  pid = fork();
     1b7:	e8 8e 0b 00 00       	call   d4a <fork>
  if(pid == -1)
...
fork1(void)
{
  ...
  pid = fork();
     1cd:	e8 78 0b 00 00       	call   d4a <fork>
  if(pid == -1)
    ...
    close(p[0]);
     ...
     1e9:	e8 8c 0b 00 00       	call   d7a <close>
    close(p[1]);
     1ee:	58                   	pop    %eax
     1ef:	ff 75 f4             	pushl  -0xc(%ebp)
     1f2:	e8 83 0b 00 00       	call   d7a <close>
    wait();
     1f7:	e8 5e 0b 00 00       	call   d5a <wait>
    wait();
     1fc:	e8 59 0b 00 00       	call   d5a <wait>
    break;
  ...
  if(cmd == 0)
    exit();
     204:	e8 49 0b 00 00       	call   d52 <exit>
int
fork1(void)
{
  int pid;
  ...
  pid = fork();
     209:	e8 3c 0b 00 00       	call   d4a <fork>
  if(pid == -1)
  ...
  case EXEC:
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     229:	52                   	push   %edx
     22a:	50                   	push   %eax
     22b:	e8 5a 0b 00 00       	call   d8a <exec>
  ...
  case REDIR:
    ...
    close(rcmd->fd);
     ...
     24d:	e8 28 0b 00 00       	call   d7a <close>
    if(open(rcmd->file, rcmd->mode) < 0){
     252:	59                   	pop    %ecx
     253:	58                   	pop    %eax
     254:	ff 73 10             	pushl  0x10(%ebx)
     257:	ff 73 08             	pushl  0x8(%ebx)
     25a:	e8 33 0b 00 00       	call   d92 <open>
...
fork1(void)
{
  ...
  pid = fork();
     271:	e8 d4 0a 00 00       	call   d4a <fork>
  if(pid == -1)
  ...
  case LIST:
    ...
    if(fork1() == 0)
     ...
     27d:	74 e7                	je     266 <runcmd+0xe6>
      runcmd(lcmd->left);
    wait();
     27f:	e8 d6 0a 00 00       	call   d5a <wait>
    runcmd(lcmd->right);
     ...
     297:	e8 c4 fe ff ff       	call   160 <panic>
{
  ...
  case REDIR:
    ...
    if(open(rcmd->file, rcmd->mode) < 0){
     ...
     2b4:	e8 07 0c 00 00       	call   ec0 <printf>
      exit();
     2b9:	e8 94 0a 00 00       	call   d52 <exit>
    break;
  ...
  case PIPE:
    ...
    if(fork1() == 0){
     ...
     2ce:	6a 01                	push   $0x1
     2d0:	e8 a5 0a 00 00       	call   d7a <close>
      dup(p[1]);
     2d5:	58                   	pop    %eax
     2d6:	ff 75 f4             	pushl  -0xc(%ebp)
     2d9:	e8 ec 0a 00 00       	call   dca <dup>
      close(p[0]);
     2de:	58                   	pop    %eax
     2df:	ff 75 f0             	pushl  -0x10(%ebp)
     2e2:	e8 93 0a 00 00       	call   d7a <close>
      close(p[1]);
     2e7:	58                   	pop    %eax
     2e8:	ff 75 f4             	pushl  -0xc(%ebp)
     2eb:	e8 8a 0a 00 00       	call   d7a <close>
      runcmd(pcmd->left);
     2f0:	58                   	pop    %eax
    ...
    if(fork1() == 0){
     ...
     2fc:	6a 00                	push   $0x0
     2fe:	e8 77 0a 00 00       	call   d7a <close>
      dup(p[0]);
     303:	5a                   	pop    %edx
     304:	ff 75 f0             	pushl  -0x10(%ebp)
     307:	e8 be 0a 00 00       	call   dca <dup>
      close(p[0]);
     30c:	59                   	pop    %ecx
     30d:	ff 75 f0             	pushl  -0x10(%ebp)
     310:	e8 65 0a 00 00       	call   d7a <close>
      close(p[1]);
     315:	58                   	pop    %eax
     316:	ff 75 f4             	pushl  -0xc(%ebp)
     319:	e8 5c 0a 00 00       	call   d7a <close>
      runcmd(pcmd->right);
     31e:	58                   	pop    %eax
...
fork1(void)
{
  ...
  pid = fork();
     336:	e8 0f 0a 00 00       	call   d4a <fork>
  if(pid == -1)
     ...
     4a0:	83 c7 01             	add    $0x1,%edi
{
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     4b1:	50                   	push   %eax
     4b2:	68 90 18 00 00       	push   $0x1890
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     505:	50                   	push   %eax
     506:	68 90 18 00 00       	push   $0x1890
  ...
  *ps = s;
  ...
  default:
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     556:	50                   	push   %eax
     557:	68 88 18 00 00       	push   $0x1888
  ...
  default:
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     575:	50                   	push   %eax
     576:	68 90 18 00 00       	push   $0x1890
...
peek(char **ps, char *es, char *toks)
{
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     61d:	50                   	push   %eax
     61e:	68 90 18 00 00       	push   $0x1890
...
gets(char *buf, int max)
{
  ...
  for(i=0; i+1 < max; ){
    ...
    cc = read(0, &c, 1);
     ...
     c05:	57                   	push   %edi
     c06:	6a 00                	push   $0x0
     c08:	e8 5d 01 00 00       	call   d6a <read>
    if(cc < 1)
...
stat(char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     c6d:	e8 20 01 00 00       	call   d92 <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     c81:	50                   	push   %eax
     c82:	e8 23 01 00 00       	call   daa <fstat>
     c87:	89 c6                	mov    %eax,%esi
  close(fd);
     c89:	89 1c 24             	mov    %ebx,(%esp)
     c8c:	e8 e9 00 00 00       	call   d7a <close>
  return r;
...
atoi(const char *s)
{
  ...
  n = 0;
	...
	if(*s=='-'){
     cb8:	0f be 13             	movsbl (%ebx),%edx
     cbb:	80 fa 2d             	cmp    $0x2d,%dl
     cbe:	74 38                	je     cf8 <atoi+0x48>
		sign=1;
		s++;
	}

  while('0' <= *s && *s <= '9')
     cc0:	8d 42 d0             	lea    -0x30(%edx),%eax
...
atoi(const char *s)
{
  ...
  while('0' <= *s && *s <= '9')
     cc5:	3c 09                	cmp    $0x9,%al
     cc7:	77 47                	ja     d10 <atoi+0x60>
	int sign=0;	// let positive as 0, negative as 1
  ...
  while('0' <= *s && *s <= '9')
     cda:	0f be 13             	movsbl (%ebx),%edx
     cdd:	8d 4a d0             	lea    -0x30(%edx),%ecx
     ce0:	80 f9 09             	cmp    $0x9,%cl
     ce3:	76 eb                	jbe    cd0 <atoi+0x20>
     ce5:	89 c2                	mov    %eax,%edx
     ce7:	f7 da                	neg    %edx
     ce9:	83 fe 01             	cmp    $0x1,%esi
     cec:	0f 44 c2             	cmove  %edx,%eax
  if(sign==1){
		return n*(-1);
	}
  ...
  while('0' <= *s && *s <= '9')
     cf8:	0f be 53 01          	movsbl 0x1(%ebx),%edx
  ...
  n = 0;
	...
	if(*s=='-'){
		sign=1;
		s++;
     cfc:	8d 43 01             	lea    0x1(%ebx),%eax
	}

  while('0' <= *s && *s <= '9')
     cff:	8d 4a d0             	lea    -0x30(%edx),%ecx
     d02:	80 f9 09             	cmp    $0x9,%cl
     d05:	77 09                	ja     d10 <atoi+0x60>
  ...
  n = 0;
	...
	if(*s=='-'){
		sign=1;
     d09:	be 01 00 00 00       	mov    $0x1,%esi
     d0e:	eb b9                	jmp    cc9 <atoi+0x19>
		s++;
	}

  while('0' <= *s && *s <= '9')
     d10:	31 c0                	xor    %eax,%eax
     d12:	eb db                	jmp    cef <atoi+0x3f>
     d14:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
     d1a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi
...
//////////////////////
SYSCALL(setnice)
     dfa:	b8 18 00 00 00       	mov    $0x18,%eax
     dff:	cd 40                	int    $0x40
     e01:	c3                   	ret    

00000e02 <getnice>:
SYSCALL(getnice)
     e02:	b8 17 00 00 00       	mov    $0x17,%eax
     e07:	cd 40                	int    $0x40
     e09:	c3                   	ret    

00000e0a <ps>:
SYSCALL(ps)
     e0a:	b8 19 00 00 00       	mov    $0x19,%eax
     e0f:	cd 40                	int    $0x40
     e11:	c3                   	ret    
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ...
     e98:	e8 d5 fe ff ff       	call   d72 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ...
     f08:	50                   	push   %eax
     f09:	57                   	push   %edi
     f0a:	e8 63 fe ff ff       	call   d72 <write>
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ...
     f6a:	50                   	push   %eax
     f6b:	57                   	push   %edi
     f6c:	e8 01 fe ff ff       	call   d72 <write>
     ...
     f81:	e8 ec fd ff ff       	call   d72 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ffe:	50                   	push   %eax
     fff:	57                   	push   %edi
    1000:	e8 6d fd ff ff       	call   d72 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    104f:	50                   	push   %eax
    1050:	57                   	push   %edi
    1051:	e8 1c fd ff ff       	call   d72 <write>
